...
data PageDataFill = PD_FillCourseAssessment CourseKey String | PD_FillGroupAssessment GroupKey String
...
fillGroupAssessmentPreviewPage = do
  title <- getParameter titleParam
  gk <- getParameter $ customGroupKeyPrm groupKeyParamName
  return $ fillAssessmentTemplate $ PD_FillGroupAssessment gk title
...
fillCourseAssessmentPreviewPage = do
  title <- getParameter titleParam
  ck <- getParameter $ customCourseKeyPrm courseKeyParamName
  return $ fillAssessmentTemplate $ PD_FillCourseAssessment ck title
...
fillAssessmentTemplate pdata = do
  ...
  return $ do
    Bootstrap.rowColMd12 $ do      
      postForm (routeOf assessment) $ do
        Bootstrap.textInput "n1" "Title" title
        Bootstrap.row $ do
  ...
  where
    title = case pdata of
              PD_FillCourseAssessment _ck title -> title
              PD_FillGroupAssessment _gk title -> title
    assessment = case pdata of
                   PD_FillCourseAssessment ck _title -> Pages.newCourseAssessment ck ()
                   PD_FillGroupAssessment gk _title -> Pages.newGroupAssessment gk ()
    fill = case pdata of
             PD_FillCourseAssessment ck _title -> Pages.fillCourseAssessmentPreview ck ()
             PD_FillGroupAssessment gk _title -> Pages.fillGroupAssessmentPreview gk ()
